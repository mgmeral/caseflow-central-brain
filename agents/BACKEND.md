...
`caseflow-be` has no upstream backend dependency of its own — it is the root of the dependency graph (see [../docs/architecture/system-overview.md](../docs/architecture/system-overview.md)). Changes here are the ones most likely to require coordinated changes in `caseflow-fe`, `caseflow-mobile`, and `caseflow-ai-service`.
