...
| **Local (Docker Compose)** | `docker compose up -d` — app:8080, postgres:5432, mongo:27017, minio:9000/9001 | Docker image built with `VITE_API_URL`/`VITE_USE_MOCKS` build args | `docker-compose up --build` — service:8081, ollama:11434, qdrant:6333/6334 | n/a (device/simulator only) |
