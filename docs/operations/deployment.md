...
- `caseflow-mobil`: Expo build (`npm run android` / `ios` / `web`); `EXPO_PUBLIC_*` env vars.

## Release coordination
- `caseflow-be` is the dependency root: deploy/verify it first when a change touches the API contract, then `caseflow-fe`/`caseflow-mobil`, then `caseflow-ai-service` if the ingest/query contract changed.
