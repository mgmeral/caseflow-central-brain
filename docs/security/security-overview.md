...
## Security baseline
- **Auth**: JWT Bearer tokens issued by `caseflow-be` (`POST /api/auth/login`). Access token 1h, refresh token 7d, DB-backed and hashed (SHA-256), rotated on every refresh (old refresh token revoked immediately). `caseflow-fe` and `caseflow-mobile` use the identical contract — no separate mobile auth flow.
